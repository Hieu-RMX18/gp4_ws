...
| Reorg compiles and tests | `colcon build --symlink-install`, sandboxed `colcon test`, and `colcon test-result --verbose` report 2901 tests, 0 errors, 0 failures, 614 skipped. |
| HMI frontend/backend remain buildable | `/usr/bin/python3 -m pytest hmi/backend/tests -q` reports 190 passed, 6 skipped. `npm run build` from `hmi/frontend` passes. TCP-socket HMI E2E tests are still present but not counted as executed in this backend run. |
