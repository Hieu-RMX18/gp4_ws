...
class CommandE2ESimTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_supported_hmi_commands_execute_in_sim(self) -> None:
        ...
        for index, case in enumerate(cases, start=1):
            with self.subTest(intent=case["intent_text"]):
                ...
                self.assertEqual(
                    result["command"]["executionResult"]["status"], "succeeded"
                )
    ...
    async def _run_command_case(
        ...
        *,
        ...
        self.assertEqual(
            command_response["command"]["parsedIntent"]["action"], expected_action
        )
        ...
        self.assertTrue(
            confirm_response["accepted"], msg=json.dumps(confirm_response, indent=2)
        )
        self.assertEqual(
        ...
        replay_payload = await asyncio.to_thread(
            self._get_json, "/api/hmi/replay?limit=25"
        )
    ...
    async def _run_sequence_case(
        ...
        *,
        ...
        self.assertEqual(
            sequence_response["sequence"]["lifecycleState"], "NEEDS_CONFIRMATION"
        )
        if expected_actions is not None:
            self.assertEqual(
                [
                    step["parsedIntent"]["action"]
                    for step in sequence_response["sequence"]["steps"]
                ],
                expected_actions,
            )
        else:
        ...
        if expected_macro_name is not None:
            self.assertEqual(
                sequence_response["sequence"]["planSummary"]["macroName"],
                expected_macro_name,
            )
        ...
        if expected_macro_name is not None:
            self.assertEqual(
                sequence_payload["planSummary"]["macroName"], expected_macro_name
            )
        self.assertTrue(sequence_payload["steps"])
        self.assertTrue(
            all(step["finalState"] == "SUCCEEDED" for step in sequence_payload["steps"])
        )
    ...
    def _assert_process_alive(
        self, process: subprocess.Popen[str] | None, label: str
    ) -> None:
        if process is None:
