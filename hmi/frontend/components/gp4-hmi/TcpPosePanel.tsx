...
export function TcpPosePanel({ toolPose }: TcpPosePanelProps) {
  ...
  return (
    <div className="tcp-pose-panel">
      ...
      <div className="tcp-row" style={{ marginTop: '8px' }}>
        <span className="tcp-label" style={{ fontWeight: 'bold' }}>tool0 XYZ</span>
        <span className="tcp-value" style={{ fontWeight: 'bold', color: '#10b981' }}>{tool0Fmt.xyz}</span>
      </div>
      <div className="tcp-row">
        <span className="tcp-label">tool0 RPY</span>
        <span className="tcp-value">{tool0Fmt.rpy}</span>
      </div>
