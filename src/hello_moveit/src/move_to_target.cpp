 ...
 * move_to_target.cpp
 * ──────────────────
 * Di chuyển end effector của GP4 đến tọa độ Cartesian (x, y, z) yêu cầu.
 *
 * Tính năng an toàn:
 ...
 *
 * Cách dùng:
 *   ros2 run hello_moveit move_to_target -- <x> <y> <z>
 *   ros2 run hello_moveit move_to_target -- <x> <y> <z> <ox> <oy> <oz> <ow>
 *
 * Ví dụ:
 *   ros2 run hello_moveit move_to_target -- 0.3 0.2 0.4
 *   ros2 run hello_moveit move_to_target -- 0.3 0.2 0.4 0.0 0.0 0.707 0.707
 */

// ── Workspace bounds (mét) ──────────────────────────────────────────
static constexpr double WS_X_MIN = -0.5;
static constexpr double WS_X_MAX = 0.5;
static constexpr double WS_Y_MIN = -0.5;
static constexpr double WS_Y_MAX = 0.5;
static constexpr double WS_Z_MIN = 0.1;
static constexpr double WS_Z_MAX = 0.5;
...
int main(int argc, char* argv[])
{
  ...
  if (!validate_target(target_x, target_y, target_z, logger))
  {
    ...
    RCLCPP_ERROR(logger, "Workspace: x[%.2f,%.2f] y[%.2f,%.2f] z[%.2f,%.2f]", WS_X_MIN, WS_X_MAX, WS_Y_MIN, WS_Y_MAX,
                 WS_Z_MIN, WS_Z_MAX);
    rclcpp::shutdown();
    return 1;
  }

  // ── 3. Bật Trajectory Mode trên MotoROS2 ─────────────────────────
  auto drive = rclcpp::Node::make_shared("move_to_target_enable");
  ...
  if (enable_client->wait_for_service(std::chrono::seconds(2)))
  {
    ...
    RCLCPP_INFO(logger, "[SAFETY] Sending start_traj_mode request...");
    if (rclcpp::spin_until_future_complete(drive, future) == rclcpp::FutureReturnCode::SUCCESS)
    {
      RCLCPP_INFO(logger, "[SAFETY] Trajectory mode enabled OK.");
    }
    else
    {
      RCLCPP_ERROR(logger, "[SAFETY] Failed to enable trajectory mode!");
    }
  }
  else
  {
    RCLCPP_WARN(logger, "[SAFETY] Service yaskawa/start_traj_mode not available (simulation mode?).");
  }

  // ── 4. Khởi tạo MoveGroupInterface ──────────────────────────────
  auto const node = std::make_shared<rclcpp::Node>(
      "move_to_target", rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true));
  ...
  RCLCPP_INFO(logger, "    Position:    (%.4f, %.4f, %.4f)", current_pose.pose.position.x, current_pose.pose.position.y,
              current_pose.pose.position.z);
  RCLCPP_INFO(logger, "    Orientation: (%.4f, %.4f, %.4f, %.4f)", current_pose.pose.orientation.x,
  ...
  //
  move_group.setWorkspace(WS_X_MIN, WS_Y_MIN, WS_Z_MIN, WS_X_MAX, WS_Y_MAX, WS_Z_MAX);
  RCLCPP_INFO(logger, "[SAFETY] Workspace: x[%.2f,%.2f] y[%.2f,%.2f] z[%.2f,%.2f]", WS_X_MIN, WS_X_MAX, WS_Y_MIN,
              WS_Y_MAX, WS_Z_MIN, WS_Z_MAX);
  ...
  RCLCPP_INFO(logger, "[SAFETY] Velocity: %.0f%%, Acceleration: %.0f%%, Planning time: %.1fs", VELOCITY_SCALE * 100,
              ACCELERATION_SCALE * 100, PLANNING_TIME);
  ...
  //
  // ── 9. Đặt Target Pose & Tìm nghiệm IK tối ưu (tránh lật khớp) ────────
  ...
  // Tính IK cho target_pose, xuất phát từ current_state với timeout 1 giây
  ...
  // Chuyển sang planner ưu tiên đường đi ngắn nhất thay vì ngẫu nhiên
  move_group.setPlannerId("RRTstar");
  RCLCPP_INFO(logger, "Target set: (%.4f, %.4f, %.4f) orientation (%.4f, %.4f, %.4f, %.4f)", target_x, target_y,
              target_z, orient_x, orient_y, orient_z, orient_w);
  ...
  if (exec_result == moveit::core::MoveItErrorCode::SUCCESS)
  {
    ...
    RCLCPP_INFO(logger, "    Position:    (%.4f, %.4f, %.4f)", final_pose.pose.position.x, final_pose.pose.position.y,
                final_pose.pose.position.z);
    RCLCPP_INFO(logger, "    Orientation: (%.4f, %.4f, %.4f, %.4f)", final_pose.pose.orientation.x,
                final_pose.pose.orientation.y, final_pose.pose.orientation.z, final_pose.pose.orientation.w);
    RCLCPP_INFO(logger, "═══════════════════════════════════════════════════");
  }
  else
  {
