...
struct RecoveryCallbacks
{
  ...
  /// Verify joint state is fresh — returns true if /joint_states is recently received.
...
public:
  ...
  /// Execute the full recovery sequence: FAILED → STOP → RESET → VERIFY → READY.
  /// Blocking call. Returns when recovery succeeds or fails.
