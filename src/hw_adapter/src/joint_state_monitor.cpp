...
void JointStateMonitor::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  if (!msg)
  {
  ...
  if (msg->name.empty())
  {
    next_snapshot.status_message = "invalid joint state: message contains no joint names";
  }
  ...
  else
  {
    ...
    {
      const auto it = std::find(msg->name.begin(), msg->name.end(), expected_name);
      if (it == msg->name.end())
      {
      ...
      {
        next_snapshot.status_message =
          "invalid joint state: non-finite position for joint '" + expected_name + "'";
        mapping_ok = false;
        break;
      }
