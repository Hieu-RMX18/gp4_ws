...
template <typename T>
T declare_or_get_parameter(rclcpp::Node &node, const std::string &name,
...
BackendCapabilities::BackendCapabilities() { initialize_defaults(); }
...
bool BackendCapabilities::validate_joint_names(
    const std::vector<std::string> &joint_names, std::string &reason) const {
...
void BackendCapabilities::initialize_defaults() {
  snapshot_.joint_names = {"joint_1_s", "joint_2_l", "joint_3_u",
...
void BackendCapabilities::detect_runtime_properties(rclcpp::Node &node) {
  ...
  snapshot_.effort_feedback_zero_expected = declare_or_get_parameter<bool>(
      node, "backend.expects_zero_effort_feedback",
      snapshot_.effort_feedback_zero_expected);
  snapshot_.open_loop_control_supported =
      declare_or_get_parameter<bool>(node, "backend.supports_open_loop_control",
                                     snapshot_.open_loop_control_supported);
  snapshot_.supports_async_execution =
      declare_or_get_parameter<bool>(node, "backend.supports_async_motion",
                                     snapshot_.supports_async_execution);
}
} // namespace hw_adapter
