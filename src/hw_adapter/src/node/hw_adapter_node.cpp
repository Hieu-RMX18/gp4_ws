...
// === Merged from hw_adapter_dispatch.cpp ===
rclcpp_action::GoalResponse HwAdapterNode::handle_dispatch_goal(
  ...
  if (!goal || goal->trajectory.points.empty()) {
    RCLCPP_WARN(get_logger(),
                "Rejecting DispatchTrajectory: empty trajectory.");
...

  {
    ...
    last_status_message_ =
        "dispatch goal accepted; reserving single execution slot";
  }

  RCLCPP_INFO(get_logger(), "Accepted DispatchTrajectory goal_id=%s points=%zu",
              goal_uuid_to_string(uuid).c_str(),
  ...
  while (it != dispatch_worker_futures_.end()) {
    if (it->valid() &&
      ...
      try {
        ...
        RCLCPP_ERROR(get_logger(), "Dispatch worker ended with exception: %s",
                     ex.what());
      } catch (...) {
        RCLCPP_ERROR(get_logger(),
                     "Dispatch worker ended with unknown exception.");
      }
...
void HwAdapterNode::wait_for_dispatch_workers() {
  ...
  for (auto &worker : workers) {
    ...
    try {
      ...
      RCLCPP_ERROR(get_logger(),
                   "Dispatch worker join failed with unknown exception.");
    }
  }
}
...
void HwAdapterNode::execute_dispatch(
  ...
  const std::string dispatch_goal_id =
      goal_uuid_to_string(goal_handle->get_goal_id());
  ...
  const double timeout_sec =
      (goal->timeout_sec > 0.0) ? goal->timeout_sec : 30.0;
  ...
  RCLCPP_INFO(
      ...
      dispatch_goal_id.c_str(), timeout_sec, goal->trajectory.points.size());
  ...
  std::thread cancel_watcher([this, goal_handle, &execution_finished,
                              &cancel_stop_requested, dispatch_goal_id]() {
    while (!execution_finished.load() && !shutdown_requested_.load()) {
      if (goal_handle->is_canceling()) {
        cancel_stop_requested.store(true);
        std::string stop_reason;
        const bool stopped =
            session_manager_ && session_manager_->stop_motion(stop_reason);
        RCLCPP_WARN(get_logger(),
                    "DispatchTrajectory goal_id=%s cancel watcher stop_motion "
                    "attempted=%s success=%s detail=%s",
  ...
  if (report.success) {
    ...
    RCLCPP_INFO(
        ...
        dispatch_goal_id.c_str(), report.message.c_str());
...
// === Merged from hw_adapter_services.cpp ===
