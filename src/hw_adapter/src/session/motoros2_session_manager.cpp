...
namespace {
using hw_adapter::timeout_reason;

template <typename ServiceT>
bool wait_for_service_or_timeout(
    const typename rclcpp::Client<ServiceT>::SharedPtr &client,
    const std::string &service_name, const std::chrono::milliseconds timeout,
    std::string &reason) {
  if (!client) {
...
bool wait_for_future_or_timeout(
    ...
    const FutureT &future, const std::chrono::milliseconds timeout,
    const std::string &contract_name, std::string &reason) {
  const auto result =
      rclcpp::spin_until_future_complete(node_base, future, timeout);
...
Motoros2SessionManager::Motoros2SessionManager(
    ...
    : logger_(node.get_logger()), service_names_(std::move(service_names)),
  ...
  snapshot_.motoros2_interfaces_available =
      HW_ADAPTER_HAS_MOTOROS2_SESSION_SERVICES != 0;

#if HW_ADAPTER_HAS_MOTOROS2_SESSION_SERVICES
  if (!service_names_.start_traj_mode.empty()) {
    start_traj_mode_client_ = client_node_->create_client<StartTrajMode>(
        service_names_.start_traj_mode);
  }
  if (!service_names_.reset_error.empty()) {
    reset_error_client_ =
        client_node_->create_client<ResetError>(service_names_.reset_error);
  }
#endif
  ...
  if (!service_names_.stop_motion.empty()) {
    stop_motion_client_ =
        client_node_->create_client<StopMotion>(service_names_.stop_motion);
  }
#endif
  ...
  if (!service_names_.follow_joint_trajectory_action.empty()) {
    follow_joint_trajectory_client_ =
        rclcpp_action::create_client<FollowJointTrajectory>(
            client_node_, service_names_.follow_joint_trajectory_action);
  }
  ...
  snapshot_.status_message =
      snapshot_.motoros2_interfaces_available
          ...
          : "MotoROS2 session services are unavailable in this build "
            "environment";
}
...
bool Motoros2SessionManager::wait_for_required_services(
    std::chrono::milliseconds timeout, std::string &reason) const {
...
#else
  if (!service_names_.start_traj_mode.empty() &&
      !wait_for_service_or_timeout<StartTrajMode>(
          start_traj_mode_client_, service_names_.start_traj_mode, timeout,
          reason)) {
    return false;
  }

  if (!service_names_.reset_error.empty() &&
      !wait_for_service_or_timeout<ResetError>(
          reset_error_client_, service_names_.reset_error, timeout, reason)) {
    return false;
  }
#endif
  ...
  if (stop_motion_client_ != nullptr &&
      !wait_for_service_or_timeout<StopMotion>(
          stop_motion_client_, service_names_.stop_motion, timeout, reason)) {
    return false;
  }
#endif
  ...
  if (follow_joint_trajectory_client_ != nullptr &&
    ...
    reason =
        timeout_reason("action server",
                       service_names_.follow_joint_trajectory_action, timeout);
    return false;
  }
...
bool Motoros2SessionManager::start_traj_mode(std::string &reason) {
...
#if !HW_ADAPTER_HAS_MOTOROS2_SESSION_SERVICES
  reason = "StartTrajMode is unavailable because motoros2_interfaces was not "
           "found at build time";
  ...
  }
  {
    ...
    if (snapshot_.session_ready) {
      reason.clear();
      snapshot_.status_message =
          "Trajectory mode already active for this adapter session";
      return true;
    }
  }

  if (!wait_for_service_or_timeout<StartTrajMode>(
          start_traj_mode_client_, service_names_.start_traj_mode,
          operation_timeout_, reason)) {
    update_status_message(reason);
    return false;
  }
  ...
  if (!wait_for_future_or_timeout(client_node_->get_node_base_interface(),
                                  future, operation_timeout_,
                                  service_names_.start_traj_mode, reason)) {
    update_status_message(reason);
    return false;
  }
  ...
  if (response->result_code.value ==
      motoros2_interfaces::msg::MotionReadyEnum::READY) {
    set_session_ready(true, response->message.empty() ? "Trajectory mode ready"
                                                      : response->message);
    reason.clear();
    return true;
  }
  ...
  if (response->result_code.value ==
      motoros2_interfaces::msg::MotionReadyEnum::NOT_READY_ESTOP) {
    oss << ". E-stop active: prefer stop_motion before retrying "
           "start_traj_mode.";
  }
  reason = oss.str();
  update_status_message(reason);
...
bool Motoros2SessionManager::reset_error(std::string &reason) {
...
#if !HW_ADAPTER_HAS_MOTOROS2_SESSION_SERVICES
  reason = "ResetError is unavailable because motoros2_interfaces was not "
           "found at build time";
  update_status_message(reason);
  return false;
#else
  if (!wait_for_service_or_timeout<ResetError>(reset_error_client_,
  ...
  if (!wait_for_future_or_timeout(client_node_->get_node_base_interface(),
                                  future, operation_timeout_,
                                  service_names_.reset_error, reason)) {
    update_status_message(reason);
    return false;
  }
  ...
  if (response->result_code.value ==
      motoros2_interfaces::msg::MotionReadyEnum::READY) {
    set_session_ready(false, response->message.empty() ? "ResetError succeeded"
                                                       : response->message);
    reason.clear();
    return true;
  }
...
bool Motoros2SessionManager::stop_motion(std::string &reason) {
#if HW_ADAPTER_HAS_STOP_MOTION_SERVICE
  {
    ...
    if (stop_motion_client_ != nullptr) {
      if (!wait_for_service_or_timeout<StopMotion>(
              stop_motion_client_, service_names_.stop_motion,
              operation_timeout_, reason)) {
        update_status_message(reason);
        return false;
      }
      ...
      if (!wait_for_future_or_timeout(client_node_->get_node_base_interface(),
                                      future, operation_timeout_,
                                      service_names_.stop_motion, reason)) {
        update_status_message(reason);
        return false;
      }
      ...
      if (response->code.val ==
          industrial_msgs::msg::ServiceReturnCode::SUCCESS) {
      ...
      std::ostringstream oss;
      oss << "StopMotion failed with return code "
          << static_cast<int>(response->code.val);
      reason = oss.str();
      update_status_message(reason);
      return false;
    }
  ...
  std::lock_guard<std::mutex> call_lock(call_mutex_);
  if (follow_joint_trajectory_client_ == nullptr) {
    reason = "stop_motion is unavailable: no StopMotion service client and no "
  ...
  if (!follow_joint_trajectory_client_->wait_for_action_server(
          operation_timeout_)) {
    reason = timeout_reason("action server",
  ...
  if (!wait_for_future_or_timeout(
          client_node_->get_node_base_interface(), future, operation_timeout_,
          service_names_.follow_joint_trajectory_action +
              "/_action/cancel_goal",
          reason)) {
    update_status_message(reason);
  ...
  if (response->return_code ==
    ...
    set_session_ready(false,
                      "stop_motion completed via FollowJointTrajectory cancel");
    reason.clear();
    return true;
  }
...
void Motoros2SessionManager::set_session_ready(const bool ready,
