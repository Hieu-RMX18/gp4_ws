...
const char *recovery_state_name(RecoveryState state) {
  ...
  case RecoveryState::STOPPING:
    return "STOPPING";
  case RecoveryState::RESETTING_ERROR:
    return "RESETTING_ERROR";
  case RecoveryState::VERIFYING_JOINT_STATE:
    return "VERIFYING_JOINT_STATE";
  case RecoveryState::READY:
    return "READY";
  case RecoveryState::RECOVERY_FAILED:
...
RecoveryStateMachine::RecoveryStateMachine(
    ...
    : logger_(logger), callbacks_(std::move(callbacks)),
      step_timeout_(step_timeout), total_timeout_(total_timeout) {}
...
RecoveryResult RecoveryStateMachine::execute() {
  ...
  RCLCPP_WARN(logger_,
              "J4-Recovery: starting recovery sequence (step_timeout=%ldms, "
              "total_timeout=%ldms)",
  ...
  auto check_total_timeout = [&]() -> bool {
    if (total_elapsed_ms() >= total_timeout_) {
      RCLCPP_ERROR(logger_, "J4-Recovery: total timeout exceeded at state %s",
  ...
  // ---- Step 1: STOP ----
  {
    ...
    if (!transition_stop(reason)) {
      ...
      // the controller may already be stopped, or stop_motion may not be
      // configured.
      RCLCPP_WARN(
          logger_,
  ...
  // ---- Step 2: RESET_ERROR ----
  {
    ...
    RCLCPP_INFO(logger_,
                "J4-Recovery: [2/4] RESETTING_ERROR — calling reset_error");
    std::string reason;
    if (!transition_reset_error(reason)) {
      RCLCPP_ERROR(logger_, "J4-Recovery: reset_error FAILED: %s",
                   reason.c_str());
      state_ = RecoveryState::RECOVERY_FAILED;
  ...
  // ---- Step 3: VERIFY_JOINT_STATE ----
  {
    ...
    if (!transition_verify_joint_state(reason)) {
      RCLCPP_ERROR(logger_, "J4-Recovery: joint_state verification FAILED: %s",
                   reason.c_str());
      state_ = RecoveryState::RECOVERY_FAILED;
  ...
  {
    RCLCPP_INFO(logger_,
                "J4-Recovery: [4/4] READY check — is_ready_for_motion");
    std::string reason;
    if (!transition_ready_check(reason)) {
      RCLCPP_ERROR(logger_, "J4-Recovery: READY check FAILED: %s",
                   reason.c_str());
      state_ = RecoveryState::RECOVERY_FAILED;
...
finalize:
  ...
  if (result.recovered) {
    RCLCPP_INFO(logger_, "J4-Recovery: SUCCESS in %.2fs (%d/4 steps)",
                result.elapsed_sec, result.steps_completed);
  } else {
    RCLCPP_ERROR(logger_,
                 "J4-Recovery: FAILED at state %s after %.2fs (%d/4 steps): %s",
                 recovery_state_name(state_), result.elapsed_sec,
                 result.steps_completed, result.message.c_str());
  }
