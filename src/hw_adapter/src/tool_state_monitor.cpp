...
ToolStateMonitor::ToolStateMonitor(rclcpp::Node &node,
    ...
    : logger_(node.get_logger()), service_names_(std::move(service_names)),
      tool_io_address_(tool_io_address), poll_period_(poll_period),
      request_timeout_(
          std::max(std::chrono::milliseconds(250), poll_period * 2)) {
  snapshot_.motoros2_interfaces_available =
      HW_ADAPTER_HAS_TOOL_IO_INTERFACES != 0;
...
#if !HW_ADAPTER_HAS_TOOL_IO_INTERFACES
  snapshot_.status_message = "tool IO monitoring unavailable: "
                             "motoros2_interfaces was not found at build time";
  (void)node;
#else
  if (!snapshot_.read_service_configured) {
    snapshot_.status_message = "tool IO monitoring unavailable: read_single_io "
  ...
  if (tool_io_address_ == 0U) {
    snapshot_.status_message = "tool IO monitoring unavailable: "
                               "tool_io_address must be greater than zero";
    return;
  }

  if (poll_period_.count() <= 0) {
    snapshot_.status_message = "tool IO monitoring unavailable: "
                               "tool_poll_period_ms must be greater than zero";
    return;
  }

  read_client_ =
      node.create_client<ReadSingleIO>(service_names_.read_single_io);
  if (snapshot_.write_service_configured) {
    write_client_ =
        node.create_client<WriteSingleIO>(service_names_.write_single_io);
  }
  ...
  poll_timer_ = node.create_wall_timer(
      poll_period_, std::bind(&ToolStateMonitor::polling_timer_callback, this));
#endif
}
...
void ToolStateMonitor::polling_timer_callback() {
  ...
  if (!read_client_) {
    set_unknown_state(
        "tool IO monitoring unavailable: read client is not initialized");
    return;
  }
  ...
  const auto now = std::chrono::steady_clock::now();
  {
    ...
    if (request_in_flight_) {
      ...
      oss << "tool IO read timed out after " << request_timeout_.count()
          << " ms";
...
void ToolStateMonitor::handle_read_response(
    uint64_t request_sequence, ReadSingleIOClient::SharedFuture future) {
  ReadSingleIO::Response::SharedPtr response;
  try {
    response = future.get();
  } catch (const std::exception &ex) {
    std::lock_guard<std::mutex> lock(state_mutex_);
    if (!pending_request_sequence_.has_value() ||
        *pending_request_sequence_ != request_sequence) {
      return;
    }
    ...
    snapshot_.output_state = false;
    snapshot_.status_message =
        std::string("tool IO read callback failed: ") + ex.what();
    return;
  }
