  ...
  const std::chrono::milliseconds timeout)
{
  ...
  stream << "Timed out after " << timeout.count() << " ms waiting for " << contract_type
         << " '" << contract_name << "'";
  return stream.str();
}
  ...
  const TrajectoryExecutionRequest & request)
{
  ...
  if (request.enforce_start_state_match)
  {
    const auto gate = evaluate_start_state_gate(
      ...
      preflight_snapshot.current_joint_positions);
  ...
  if (request.enforce_start_state_match)
  {
    ...
    const auto drift_gate = evaluate_start_state_gate(
      preflight_snapshot.current_joint_positions,
      commit_snapshot.current_joint_positions);
    ...
    if (!drift_gate.valid)
    {
      ...
      stream << "commit-time drift recheck rejected dispatch: max_abs_delta="
             << drift_gate.max_abs_delta << " rad (limit " << start_state_max_abs_delta_rad_
  ...
  result.action_result_name = decode_action_result_name(result.action_result_code);

  result.controller_error_code = wrapped_result.result->error_code;
  result.controller_error_name = decode_controller_error_name(result.controller_error_code);
  result.controller_error_string = wrapped_result.result->error_string;

  const auto build_controller_error_message = [&result]() {
      std::ostringstream stream;
      stream << "FollowJointTrajectory reported "
  ...
  switch (wrapped_result.code)
  {
    case rclcpp_action::ResultCode::SUCCEEDED:
      if (result.controller_error_code == FollowJointTrajectory::Result::SUCCESSFUL)
      {
        result.success = true;
  ...
  std::string & reason) const
{
  ...
  {
    reason = "trajectory joint order does not match canonical GP4 joint ordering";
    return false;
  }
  ...
  for (std::size_t index = 0; index < traj.points.size(); ++index)
  {
    ...
    const std::string point_label = "trajectory point[" + std::to_string(index) + "]";
    ...
    if (!is_finite_vector(point.positions) ||
      ...
      (!point.accelerations.empty() && !is_finite_vector(point.accelerations)) ||
    ...
    if (header_stamp <= now)
    {
      ...
      if (age_ms > trajectory_header_max_age_)
      {
        ...
        stream << "trajectory header stamp is stale: age=" << age_ms.count()
               << " ms exceeds max_age=" << trajectory_header_max_age_.count() << " ms";
        reason = stream.str();
        return false;
      }
    }
  ...
  std::string & reason) const
{
  ...
  {
    reason = snapshot.session_status.empty() ?
...
TrajectoryExecutor::StartStateGateResult TrajectoryExecutor::evaluate_start_state_gate(
  ...
  const std::vector<double> & actual) const
{
  ...
  if (expected.empty())
  {
    gate.reason = "expected_start_positions is empty while start-state enforcement is enabled";
    return gate;
  }
  ...
  gate.valid =
    gate.max_abs_delta <= start_state_max_abs_delta_rad_ &&
  ...
  if (!gate.valid)
  {
    ...
    stream << "start-state continuity gate rejected dispatch: max_abs_delta="
           << gate.max_abs_delta << " rad (limit " << start_state_max_abs_delta_rad_
           << "), l2_delta=" << gate.l2_delta << " rad (limit "
           << start_state_max_l2_delta_rad_ << ")";
    gate.reason = stream.str();
  }
  ...
  const TrajectoryExecutionResult & result) const
{
  ...
  const double duration_sec = point_count > 0U ?
    rclcpp::Duration(request.trajectory.points.back().time_from_start).seconds() :
    0.0;
  const auto preflight_joint_age_ms =
    result.preflight_snapshot_available ? result.preflight_joint_state_age.count() : -1;
  const auto commit_joint_age_ms =
    result.commit_snapshot_available ? result.commit_joint_state_age.count() : -1;
  const auto preflight_robot_age_ms =
    result.preflight_snapshot_available ? result.preflight_robot_status_age.count() : -1;
  const auto commit_robot_age_ms =
    result.commit_snapshot_available ? result.commit_robot_status_age.count() : -1;

  RCLCPP_INFO(
    logger_,
    "dispatch_diagnostics{command_id=%s, primitive=%s, planner_id=%s, points=%zu, "
    "duration_sec=%.6f, preflight_joint_state_age_ms=%ld, commit_joint_state_age_ms=%ld, "
    "preflight_robot_status_age_ms=%ld, commit_robot_status_age_ms=%ld, "
    "preflight_session_ready=%s, commit_session_ready=%s, preflight_session_status=\"%s\", "
    "commit_session_status=\"%s\", start_state_max_abs_delta=%.6f, start_state_l2_delta=%.6f, "
    "commit_drift_max_abs_delta=%.6f, commit_drift_l2_delta=%.6f, failure_stage=%s, "
    "action_result_code=%d, action_result_name=%s, controller_error_code=%d, "
    "controller_error_name=%s, controller_error_string=\"%s\", message=\"%s\"}",
    ...
    point_count,
    duration_sec,
    static_cast<long>(preflight_joint_age_ms),
    ...
    result.preflight_session_status.empty() ? "<unset>" : result.preflight_session_status.c_str(),
    result.commit_session_status.empty() ? "<unset>" : result.commit_session_status.c_str(),
    result.max_start_state_abs_delta,
    result.start_state_l2_delta,
    result.commit_drift_max_abs_delta,
    result.commit_drift_l2_delta,
    result.failure_stage.c_str(),
    result.action_result_code,
    result.action_result_name.c_str(),
    result.controller_error_code,
    result.controller_error_name.c_str(),
    result.controller_error_string.empty() ? "<none>" : result.controller_error_string.c_str(),
    result.message.empty() ? "<none>" : result.message.c_str());
}

std::string TrajectoryExecutor::decode_action_result_name(const int32_t result_code)
{
...
std::string TrajectoryExecutor::decode_controller_error_name(const int32_t error_code)
{
  switch (error_code)
  {
