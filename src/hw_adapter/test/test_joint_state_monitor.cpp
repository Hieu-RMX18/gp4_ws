...
{
  auto monitor_node = std::make_shared<rclcpp::Node>("joint_state_monitor_unknown_test");
  hw_adapter::JointStateMonitor monitor(
    *monitor_node,
...
TEST_F(JointStateMonitorTest, reorders_joint_positions_to_canonical_layout)
{
  ...
  auto monitor_node = std::make_shared<rclcpp::Node>("joint_state_monitor_reordered_test");
  auto publisher_node = std::make_shared<rclcpp::Node>("joint_state_monitor_reordered_pub");
  hw_adapter::JointStateMonitor monitor(*monitor_node, canonical_joint_names(), topic, 200ms);

  auto publisher =
    publisher_node->create_publisher<sensor_msgs::msg::JointState>(topic, rclcpp::QoS(10));
  ...
  ASSERT_TRUE(spin_until(executor, [&monitor]() {return monitor.latest_snapshot().valid;}));
  const auto snapshot = monitor.latest_snapshot();
...
TEST_F(JointStateMonitorTest, stale_joint_state_is_invalid)
{
  ...
  auto monitor_node = std::make_shared<rclcpp::Node>("joint_state_monitor_stale_test");
  auto publisher_node = std::make_shared<rclcpp::Node>("joint_state_monitor_stale_pub");
  hw_adapter::JointStateMonitor monitor(*monitor_node, canonical_joint_names(), topic, 30ms);
  ...
  const auto snapshot = monitor.latest_snapshot();
  EXPECT_FALSE(snapshot.valid);
  EXPECT_NE(snapshot.status_message.find("stale joint state"), std::string::npos);
}
