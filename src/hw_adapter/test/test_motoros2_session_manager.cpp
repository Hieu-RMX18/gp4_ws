...
TEST_F(Motoros2SessionManagerTest, start_traj_mode_success_path) {
  ...
  auto server_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_start_server");
  ...
  auto client_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_start_client");
  hw_adapter::Motoros2SessionManager manager(
      *client_node,
      hw_adapter::SessionServiceNames{start_service, "/unused_reset_error", "",
                                      "/unused_action"},
      300ms);
...
TEST_F(Motoros2SessionManagerTest,
       start_traj_mode_timeout_when_service_unavailable) {
  auto client_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_timeout_client");
  hw_adapter::Motoros2SessionManager manager(
      *client_node,
      hw_adapter::SessionServiceNames{
          "/test_hw_adapter/missing_start_traj_mode", "/unused_reset_error", "",
          "/unused_action"},
      50ms);
  ...
  EXPECT_NE(reason.find("/test_hw_adapter/missing_start_traj_mode"),
            std::string::npos);
}

TEST_F(Motoros2SessionManagerTest, reset_error_call_path) {
  const std::string start_service =
      "/test_hw_adapter/start_traj_mode_for_reset";
  const std::string reset_service = "/test_hw_adapter/reset_error_success";
  auto server_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_reset_server");
  std::atomic<int> start_requests{0};
  ...
  auto reset_server = server_node->create_service<
      motoros2_interfaces::srv::ResetError>(
      reset_service,
      [&reset_requests](
          const std::shared_ptr<motoros2_interfaces::srv::ResetError::Request>,
          std::shared_ptr<motoros2_interfaces::srv::ResetError::Response>
              response) {
        ++reset_requests;
        response->result_code.value =
            motoros2_interfaces::msg::MotionReadyEnum::READY;
  ...
  auto client_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_reset_client");
  hw_adapter::Motoros2SessionManager manager(
      *client_node,
      hw_adapter::SessionServiceNames{start_service, reset_service, "",
                                      "/unused_action"},
      300ms);
...
TEST_F(Motoros2SessionManagerTest, repeated_safe_calls_do_not_corrupt_state) {
  ...
  auto server_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_repeat_server");
  std::atomic<int> start_requests{0};
  std::atomic<int> reset_requests{0};
  ...
  auto client_node =
      std::make_shared<rclcpp::Node>("motoros2_session_manager_repeat_client");
  hw_adapter::Motoros2SessionManager manager(
      *client_node,
      hw_adapter::SessionServiceNames{start_service, reset_service, "",
                                      "/unused_action"},
