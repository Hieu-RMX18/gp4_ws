...
{
  auto client_node = std::make_shared<rclcpp::Node>("tool_state_monitor_no_state_client");
  ...
  EXPECT_NE(snapshot.status_message.find("waiting for first sampled state"), std::string::npos);
}

TEST_F(ToolStateMonitorTest, state_update_case)
{
  const std::string read_service = "/test_hw_adapter/tool_read_state_update";
  auto server_node = std::make_shared<rclcpp::Node>("tool_state_monitor_state_server");
  std::atomic<int> read_requests{0};

  auto read_server = server_node->create_service<motoros2_interfaces::srv::ReadSingleIO>(
    read_service,
    [&read_requests](
      const std::shared_ptr<motoros2_interfaces::srv::ReadSingleIO::Request> request,
      std::shared_ptr<motoros2_interfaces::srv::ReadSingleIO::Response> response)
    {
      ++read_requests;
  ...
  auto client_node = std::make_shared<rclcpp::Node>("tool_state_monitor_state_client");
  hw_adapter::ToolStateMonitor monitor(
    *client_node,
    hw_adapter::ToolServiceNames{read_service, ""},
    101U,
    20ms);
  ...
  ASSERT_TRUE(wait_until([&monitor]() {return monitor.has_tool_state();}, 500ms));
...
TEST_F(ToolStateMonitorTest, unavailable_backend_case)
{
  ...
  auto client_node = std::make_shared<rclcpp::Node>("tool_state_monitor_unavailable_client");
  hw_adapter::ToolStateMonitor monitor(
    *client_node,
    hw_adapter::ToolServiceNames{read_service, ""},
    17U,
  ...
  ASSERT_TRUE(wait_until(
    [&monitor, &read_service]() {
      const auto snapshot = monitor.snapshot();
      return snapshot.status_message.find("unavailable") != std::string::npos &&
