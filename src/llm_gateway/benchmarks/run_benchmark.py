...
DEFAULT_SAFETY_RULES_PATH = (
    REPO_LLM_GATEWAY_ROOT.parent / "safety" / "config" / "safety_rules.yaml"
)
...
def _load_safety_rules() -> dict[str, Any]:
    payload = (
        yaml.safe_load(DEFAULT_SAFETY_RULES_PATH.read_text(encoding="utf-8")) or {}
    )
    if not isinstance(payload, dict):
        raise ValueError(
            f"safety rules root must be an object: {DEFAULT_SAFETY_RULES_PATH}"
        )
    return payload
...
def _l1_corpus() -> list[dict[str, Any]]:
    return [
        ...
        {
            "name": "wait",
            "payload": json.dumps({"primitive_type": "WAIT", "wait_duration_sec": 2.0}),
        },
        ...
        },
        {
            "name": "set_speed",
            "payload": json.dumps(
                {"primitive_type": "SET_SPEED", "velocity_scale": 0.05}
            ),
        },
        {
            "name": "move_rel",
...
def _run_l1() -> dict[str, Any]:
    ...
    for _ in range(DEFAULT_L1_ITERATIONS):
        for case in corpus:
            ...
            if not valid:
                raise ValueError(
                    f"L1 schema validation failed for {case['name']}: {error}"
                )
...
def _run_l2() -> dict[str, Any]:
    router = IntentRouter(
        macro_policy_path=_macro_policy_path(), runtime_mode="hardware"
    )
    corpus = _l2_corpus()
    passed_cases = 0
    ...
    for case in corpus:
        ...
        if command_fields:
            first_command = (
                result.commands[-1]
                if expected["routeType"] == "sequence"
                else result.commands[0]
            )
            for key, expected_value in command_fields.items():
                actual_value = first_command.get(key)
                if actual_value != expected_value:
                    issues.append(
                        f"field {key} expected {expected_value!r} got {actual_value!r}"
                    )
        ...
        if nested_fields:
            ...
            for key, expected_value in nested_fields.items():
                ...
                if actual_value != expected_value:
                    issues.append(
                        f"nested field {key} expected {expected_value!r} got {actual_value!r}"
                    )
...
def _parse_levels(levels_text: str) -> list[str]:
    requested = [
        level.strip().upper() for level in levels_text.split(",") if level.strip()
    ]
    if not requested:
