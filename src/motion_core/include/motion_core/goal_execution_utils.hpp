...
inline std::string make_interrupt_reason(
...
inline bool ensure_scene_ready(
  const bool require_planning_scene,
  ...
  std::string & reason)
{
  ...
  {
    stream << " (path='" << scene_objects_path << "', status="
           << scene_load_result_name(scene_load_result) << ")";
  }
  else
  {
...
inline void abort_with_message(
  const std::shared_ptr<GoalHandleExecuteMotion> & goal_handle,
  const std::chrono::steady_clock::time_point & started_at,
...
inline void cancel_with_message(
  const std::shared_ptr<GoalHandleExecuteMotion> & goal_handle,
  const std::chrono::steady_clock::time_point & started_at,
  const std::string & message)
{
