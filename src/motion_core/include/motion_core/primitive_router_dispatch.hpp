...
public:
  ...
  using PlanWithInterruptionFn = std::function<PlanningStageResult(
    ...
    const std::string &)>;
  using ReadCurrentTcpPoseFn =
    std::function<bool(geometry_msgs::msg::PoseStamped &, std::string &, double)>;
  ...
  struct PlanningRequest
  {
    ...
    double acceleration_scale = TrajectoryPostProcessor::kDefaultAccelerationScaling;
...
private:
  ...
  std::function<std::shared_ptr<moveit::planning_interface::MoveGroupInterface>()> move_group_provider_;
