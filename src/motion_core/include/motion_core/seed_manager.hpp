...
public:
  ...
  /// 3. commissioning named-target fallback (ready-style seed)
  ...
  bool get_seed_state(const std::string &primitive_family,
  ...
  bool get_current_joint_positions(std::vector<double> &positions,
                                   builtin_interfaces::msg::Time &stamp) const;

  /// V4 D2: Cache a successful IK solution for a primitive family.
  void cache_successful_seed(const std::string &primitive_family,
                             const std::vector<double> &seed);

private:
  static std::vector<std::string> default_joint_names();
  void joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg);
  bool is_joint_state_fresh_locked(std::chrono::milliseconds &age,
  ...
  bool fallback_cached_seed(const std::string &primitive_family,
                            std::vector<double> &seed) const;
