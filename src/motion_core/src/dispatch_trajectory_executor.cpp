...
DispatchTrajectoryExecutor::DispatchTrajectoryExecutor(
  ...
  std::string dispatch_action_name,
  const double dispatch_timeout_sec,
  const std::size_t safe_budget_points,
...
: logger_(logger),
  ...
  safe_budget_points_(safe_budget_points),
  quality_gate_(quality_gate),
  trajectory_post_processor_(trajectory_post_processor)
{
}

bool DispatchTrajectoryExecutor::is_geometry_sensitive_primitive(const std::string & primitive)
{
  return primitive == "LIN" || primitive == "CIRC" ||
         primitive == "MOVE_REL" || primitive == "CARTESIAN_PATH";
}

bool DispatchTrajectoryExecutor::is_finite_trajectory_vector(const std::vector<double> & values)
{
...
DispatchTrajectoryExecutor::Result DispatchTrajectoryExecutor::dispatch_to_hw_adapter(
  ...
  const UpdatePhaseFn & update_phase) const
{
  ...
  const auto query_interrupt = [&interrupt_reason](const std::string & stage) -> std::string
    {
      if (!interrupt_reason)
      {
        return {};
...
DispatchTrajectoryExecutor::Result DispatchTrajectoryExecutor::apply_budget_quality_and_dispatch(
  ...
  const UpdatePhaseFn & update_phase,
  ...
  for (std::size_t index = 0; index < segments.size(); ++index)
  {
    if (interrupt_reason)
    {
      const std::string pre_dispatch_interrupt =
        interrupt_reason("pre_dispatch_segment_" + std::to_string(index + 1U));
      if (!pre_dispatch_interrupt.empty())
      {
        result.status = Status::kCanceled;
        result.reason = pre_dispatch_interrupt;
    ...
    const auto dispatch_result =
      dispatch_to_hw_adapter(segments[index], dispatch_metadata, interrupt_reason, update_phase);
