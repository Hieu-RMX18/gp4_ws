...
NonMotionPrimitiveExecutor::NonMotionPrimitiveExecutor(
    ...
    std::string io_set_service_name, std::function<void()> stop_move_group_fn)
    : logger_(logger), execution_orchestrator_(execution_orchestrator),
...
bool NonMotionPrimitiveExecutor::handle_stop_primitive(
  ...
  const bool had_active_goal =
      execution_orchestrator_.request_stop(stop_reason);
  RCLCPP_WARN(logger_, "STOP primitive received for goal_id=%s — %s",
              goal_id.c_str(),
              had_active_goal ? stop_reason.c_str()
                              : "no active execute_motion goal to stop");
  ...
  result->message =
      had_active_goal
          ? ("STOP: motion halt requested, dispatch cancel issued (" +
             stop_reason + ")")
...
bool NonMotionPrimitiveExecutor::handle_non_motion_primitive(
    const std::string &primitive, const std::uint64_t goal_sequence,
    const std::shared_ptr<const ExecuteMotion::Goal> &goal,
    const std::shared_ptr<GoalHandleExecuteMotion> &goal_handle,
    const std::chrono::steady_clock::time_point &started_at,
    const PublishFeedbackFn &publish_feedback,
    const MessageFn &abort_with_message, const MessageFn &cancel_with_message) {
  if (primitive == "SET_SPEED") {
    execution_orchestrator_.update_phase(
        goal_sequence, ExecutionPhase::kAccepted, "set_speed");
  ...
  if (primitive == "WAIT") {
    execution_orchestrator_.update_phase(goal_sequence,
                                         ExecutionPhase::kAccepted, "wait");
    const double wait_sec = std::max(goal->wait_duration_sec, 0.0);
    RCLCPP_INFO(logger_, "WAIT goal_seq=%lu: pausing for %.3f seconds.",
                static_cast<unsigned long>(goal_sequence), wait_sec);
    ...
    while (true) {
      ...
      if (publish_feedback) {
        const double progress = std::min(
            0.1 + 0.8 * (std::chrono::duration<double>(elapsed).count() /
                         wait_sec),
            0.9);
    ...
    result->success = true;
    result->message =
        "WAIT: completed " + std::to_string(wait_sec) + " seconds";
    set_result_timing(started_at, *result);
    goal_handle->succeed(result);
    return true;
  }
  ...
  if (primitive == "ALARM_RESET") {
    execution_orchestrator_.update_phase(
        goal_sequence, ExecutionPhase::kAccepted, "alarm_reset");
    RCLCPP_INFO(logger_,
                "ALARM_RESET goal_seq=%lu: sending reset request to %s",
                static_cast<unsigned long>(goal_sequence),
                alarm_reset_service_name_.c_str());

    if (!alarm_reset_client_ ||
        !alarm_reset_client_->wait_for_service(std::chrono::seconds(5))) {
      if (abort_with_message) {
        abort_with_message("ALARM_RESET: service unavailable at " +
    ...
    if (future.wait_for(std::chrono::seconds(10)) !=
      ...
      if (abort_with_message) {
        abort_with_message("ALARM_RESET: timed out waiting for response from " +
                           alarm_reset_service_name_);
      }
      return true;
    }
    ...
    if (!response) {
      if (abort_with_message) {
        abort_with_message("ALARM_RESET: empty response from " +
                           alarm_reset_service_name_);
      }
      return true;
    }
  ...
  if (primitive == "IO_SET") {
    execution_orchestrator_.update_phase(goal_sequence,
                                         ExecutionPhase::kAccepted, "io_set");
    RCLCPP_INFO(logger_, "IO_SET goal_seq=%lu: address=%u, value=%d -> %s",
                static_cast<unsigned long>(goal_sequence), goal->io_address,
                goal->io_value, io_set_service_name_.c_str());

    if (!io_set_client_ ||
        !io_set_client_->wait_for_service(std::chrono::seconds(5))) {
      if (abort_with_message) {
        abort_with_message("IO_SET: service unavailable at " +
    ...
    if (future.wait_for(std::chrono::seconds(10)) !=
        std::future_status::ready) {
      if (abort_with_message) {
        abort_with_message("IO_SET: timed out waiting for response from " +
                           io_set_service_name_);
      }
      return true;
    }
    ...
    if (!response) {
      if (abort_with_message) {
        abort_with_message("IO_SET: empty response from " +
                           io_set_service_name_);
      }
      return true;
    }
