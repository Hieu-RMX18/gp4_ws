  ...
  const std::uint64_t goal_sequence) const
{
  ...
  if (!dependencies_.ensure_move_group || !dependencies_.ensure_move_group(move_group_reason))
  {
    result.status = Status::kAborted;
    result.message = move_group_reason.empty() ? "MoveGroup unavailable" : move_group_reason;
    return result;
  }

  std::string scene_reason;
  if (!dependencies_.ensure_scene_ready || !dependencies_.ensure_scene_ready(scene_reason))
  {
    result.status = Status::kAborted;
    result.message = scene_reason.empty() ? "planning scene unavailable" : scene_reason;
    return result;
  }
  ...
  const double velocity_scale =
    (goal->velocity_scale > 0.0) ? goal->velocity_scale :
    TrajectoryPostProcessor::kDefaultVelocityScaling;
  const double acceleration_scale =
    (goal->acceleration_scale > 0.0) ? goal->acceleration_scale :
    TrajectoryPostProcessor::kDefaultAccelerationScaling;

  auto move_group = dependencies_.move_group_provider ? dependencies_.move_group_provider() : nullptr;
  if (!move_group)
  {
    result.status = Status::kAborted;
  ...
  if (!dependencies_.build_current_robot_state ||
    !dependencies_.build_current_robot_state(
      current_robot_state,
      current_state_reason,
      &source_joint_state_stamp))
  {
    result.status = Status::kAborted;
    result.message = "failed to read current joint state: " + current_state_reason;
    return result;
  }
  ...
  {
    result.status = Status::kAborted;
    result.message = "planning group '" + dependencies_.planning_group + "' not found";
    return result;
  }
  ...
  if (dependencies_.update_phase)
  {
    dependencies_.update_phase(ExecutionPhase::kPlanning, "planning_request_prepared");
  }
  ...
  planning_request.plan_with_interruption = dependencies_.plan_with_interruption;
  planning_request.interrupt_reason = dependencies_.interrupt_reason;

  const auto planning_result = dependencies_.primitive_router_dispatch.plan_for_primitive(planning_request);
  if (planning_result.status == PrimitiveRouterDispatch::PlanningStatus::kCanceled)
  {
    result.status = Status::kCanceled;
    result.message = planning_result.is_move_joint ?
  ...
  const auto dispatch_result = dependencies_.dispatch_executor.apply_budget_quality_and_dispatch(
    planning_result.trajectory,
    planning_result.dispatch_primitive,
    dispatch_metadata,
    planning_result.cartesian_fraction,
    dependencies_.interrupt_reason,
    dependencies_.publish_feedback,
    dependencies_.update_phase,
