  ...
  while (it != worker_futures_.end()) {
    if (it->valid() &&
      ...
      try {
        ...
        RCLCPP_ERROR(logger_, "execute_motion worker ended with exception: %s",
                     ex.what());
      } catch (...) {
        RCLCPP_ERROR(logger_,
                     "execute_motion worker ended with unknown exception.");
      }
...
void ExecuteMotionActionSupport::wait_for_workers() {
  ...
  for (auto &worker : workers) {
    ...
    try {
      ...
      RCLCPP_ERROR(logger_,
                   "execute_motion worker join failed with unknown exception.");
    }
  }
}
...
rclcpp_action::GoalResponse ExecuteMotionActionSupport::handle_goal(
  ...
  if (goal->acceleration_scale > max_acceleration_scale_) {
    RCLCPP_WARN(logger_,
                ...
                goal->acceleration_scale, max_acceleration_scale_);
    return rclcpp_action::GoalResponse::REJECT;
  }
...
rclcpp_action::CancelResponse ExecuteMotionActionSupport::handle_cancel(
  ...
  RCLCPP_WARN(logger_, "Cancel requested for execute_motion goal_id=%s",
...
void ExecuteMotionActionSupport::handle_accepted(
    ...
    const ExecuteFn &execute_fn, const std::atomic<bool> &shutdown_requested) {
  cleanup_finished_workers();
  std::future<void> worker = std::async(
      std::launch::async, [goal_handle, execute_fn, &shutdown_requested]() {
...
std::string
ExecuteMotionActionSupport::normalize_primitive(std::string primitive) {
  primitive.erase(
      std::remove_if(primitive.begin(), primitive.end(),
...
bool ExecuteMotionActionSupport::is_supported_primitive(
    const std::string &primitive) {
  return primitive == "HOME" || primitive == "PTP" || primitive == "LIN" ||
         primitive == "CIRC" || primitive == "MOVE_REL" ||
         primitive == "CARTESIAN_PATH" || primitive == "SET_SPEED" ||
         primitive == "WAIT" || primitive == "STOP" ||
         primitive == "MOVE_JOINT" || primitive == "MOVE_JOINTS" ||
         primitive == "IO_SET" || primitive == "ALARM_RESET" ||
         primitive == "BLENDED_SEQUENCE";
}

bool ExecuteMotionActionSupport::is_non_motion_primitive(
    const std::string &primitive) {
...
std::string ExecuteMotionActionSupport::goal_uuid_to_string(
