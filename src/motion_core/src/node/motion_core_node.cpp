...
public:
  ...
  MotionCoreNode()
      : rclcpp::Node("motion_core_node"), seed_manager_(*this),
    ...
    dispatch_timeout_sec_ =
        declare_parameter<double>("dispatch_timeout_sec", 60.0);
    dispatch_client_ = rclcpp_action::create_client<DispatchTrajectory>(
        this, dispatch_action_name_);
    // Motion limits loaded from ROS params; launch config sources
    // safety_rules.yaml. Hardcoded defaults are fail-safe only.
    max_velocity_scale_ = declare_parameter<double>("max_velocity_scale",
    ...
    if (!safety_rules_yaml_path_.empty()) {
      try {
        const auto root = YAML::LoadFile(safety_rules_yaml_path_);
        ...
        if (limits_node && limits_node.IsMap()) {
          ...
          RCLCPP_WARN(get_logger(),
                      ...
                      "JointPositionGuard disabled",
                      safety_rules_yaml_path_.c_str());
        }
      } catch (const YAML::Exception &ex) {
        RCLCPP_ERROR(
            get_logger(),
            "Failed to load JointPositionGuard from %s: %s — guard disabled",
            safety_rules_yaml_path_.c_str(), ex.what());
      }
    } else {
      RCLCPP_WARN(get_logger(),
                  "safety_rules_yaml_path not set; JointPositionGuard disabled "
                  "(no operational limits)");
    }
    dispatch_executor_ = std::make_unique<DispatchTrajectoryExecutor>(
        get_logger(), dispatch_client_, dispatch_action_name_,
        dispatch_timeout_sec_, kTrajectorySafeBudgetPoints, quality_gate_,
    ...
    query_handler_ = std::make_unique<QueryHandler>(
        ...
        [this](geometry_msgs::msg::PoseStamped &current_stamped,
               std::string &reason, double timeout_sec) {
    ...
    primitive_router_dispatch_ = std::make_unique<PrimitiveRouterDispatch>(
        get_logger(), [this]() { return move_group_; }, planner_router_,
        orientation_filter_, seed_manager_, ik_selector_,
        trajectory_post_processor_,
        [this](geometry_msgs::msg::PoseStamped &current_stamped,
               std::string &reason, double timeout_sec) {
          return read_current_tcp_pose(current_stamped, reason, timeout_sec);
    ...
    get_pose_service_ = create_service<GetCurrentPose>(
        "/get_current_pose",
        std::bind(&QueryHandler::handle_get_current_pose, query_handler_.get(),
    ...
    alarm_reset_service_name_ = declare_parameter<std::string>(
        "alarm_reset_service_name", "/hw_adapter/alarm_reset");
    ...
    io_set_service_name_ = declare_parameter<std::string>("io_set_service_name",
                                                          "/hw_adapter/io_set");
    io_set_client_ = create_client<IoSet>(io_set_service_name_);
    non_motion_executor_ = std::make_unique<NonMotionPrimitiveExecutor>(
        get_logger(), execution_orchestrator_, dispatch_client_,
        alarm_reset_client_, alarm_reset_service_name_, io_set_client_,
        io_set_service_name_, [this]() {
          if (move_group_) {
            move_group_->stop();
          }
        });
  ...
  void initialize() {
    ...
    if (!ensure_move_group(reason)) {
      RCLCPP_ERROR(get_logger(),
                   "Failed to initialize MoveGroup in initialize(): %s",
                   reason.c_str());
    } else {
      configure_manipulability_guard();
    }
    ...
    require_planning_scene_ =
        declare_parameter<bool>("require_planning_scene", true);
    scene_objects_path_ =
        declare_parameter<std::string>("scene_objects_path", "");
    ...
    if (!scene_objects_path_.empty()) {
      ...
      if (scene_load_result_ != SceneLoadResult::OK) {
        if (require_planning_scene_) {
          RCLCPP_ERROR(
              get_logger(),
              "Planning scene load failed (%s) from '%s' and "
          ...
          RCLCPP_WARN(get_logger(),
                      "Planning scene load failed (%s) from '%s' but "
                      "require_planning_scene=false; "
      ...
      if (require_planning_scene_) {
        RCLCPP_ERROR(get_logger(), "No scene_objects_path configured while "
                                   "require_planning_scene=true; "
                                   "planning is blocked (fail-closed).");
      } else {
        RCLCPP_WARN(get_logger(), "No scene_objects_path configured — planning "
                                  "scene empty (degraded mode allowed).");
      }
    }

    action_server_ = rclcpp_action::create_server<ExecuteMotion>(
        this, "execute_motion",
        std::bind(&MotionCoreNode::handle_goal, this, std::placeholders::_1,
                  std::placeholders::_2),
        std::bind(&MotionCoreNode::handle_cancel, this, std::placeholders::_1),
        std::bind(&MotionCoreNode::handle_accepted, this,
                  std::placeholders::_1));
    RCLCPP_INFO(get_logger(), "execute_motion action server ready");
  }

private:
  // Fail-safe defaults — active policy comes from ROS params set by launch
  // config.
  ...
  // V4: NO FollowJointTrajectory client. Execution goes through hw_adapter
  // only.
  ...
  ManipulabilityGuard manipulability_guard_{ManipulabilityGuard::disabled()};
  ...
  std::string safety_rules_yaml_path_;
  ...
  void configure_manipulability_guard() {
    ...
    if (safety_rules_yaml_path_.empty() || !move_group_) {
      quality_gate_ = QualityGate(kTrajectoryHardLimitPoints,
                                  QualityGate::kMinimumCartesianFraction,
                                  joint_position_guard_, manipulability_guard_);
      return;
    }

    try {
      const auto root = YAML::LoadFile(safety_rules_yaml_path_);
      const auto guard_node = root["manipulability_guard"];
      if (guard_node && guard_node.IsMap() && guard_node["enabled"] &&
          guard_node["enabled"].as<bool>()) {
        manipulability_guard_ = ManipulabilityGuard(
            move_group_->getRobotModel(), kPlanningGroup,
            guard_node["floor"].as<double>(),
            guard_node["sample_every_n_points"].as<std::size_t>(),
      ...
      RCLCPP_ERROR(get_logger(),
                   "Failed to load ManipulabilityGuard from %s: %s",
                   safety_rules_yaml_path_.c_str(), ex.what());
      manipulability_guard_ =
          ManipulabilityGuard(nullptr, kPlanningGroup, 0.0, 1);
    }
  ...
  bool ensure_move_group(std::string &reason) {
    ...
    try {
      ...
      RCLCPP_INFO(get_logger(),
                  "MoveGroup initialized; current joint state will be resolved "
                  "per-request from "
      ...
      reason =
          std::string("failed to initialize MoveGroupInterface: ") + ex.what();
      return false;
    }
  }

  bool build_current_robot_state(
      moveit::core::RobotState &current_state, std::string &reason,
    ...
    const auto *joint_model_group =
        current_state.getJointModelGroup(kPlanningGroup);
    ...
    if (current_joint_positions.size() != expected_dof) {
      ...
      stream << "latest /yaskawa/joint_states size mismatch for group '"
             << kPlanningGroup << "': expected " << expected_dof << ", got "
             << current_joint_positions.size();
      reason = stream.str();
      return false;
    }
  ...
  bool read_current_tcp_pose(geometry_msgs::msg::PoseStamped &current_stamped,
                             std::string &reason,
    ...
    if (tcp_link.empty()) {
      ...
      if (link_names.empty()) {
        reason = "planning group '" + std::string(kPlanningGroup) +
                 "' has no link models";
        return false;
      }
    ...
    const Eigen::Isometry3d &tcp_transform =
        current_state.getGlobalLinkTransform(tcp_link);
    ...
    if (!std::isfinite(translation.x()) || !std::isfinite(translation.y()) ||
      ...
      reason = "current TCP pose has non-finite position for link '" +
               tcp_link + "'";
      return false;
    }
    ...
    if (!std::isfinite(qnorm_sq) || qnorm_sq <= 1e-12) {
      reason = "current TCP pose has invalid orientation for link '" +
               tcp_link + "'";
      return false;
    }
  ...
  StageResult plan_with_interruption(
      ...
      const std::uint64_t sequence, const std::string &stage) {
    ...
    std::thread planning_thread(
        [this, &plan, &plan_code, &planning_finished, &planning_exception]() {
          try {
    ...
    while (!planning_finished.load()) {
      interrupted_reason = make_interrupt_reason(
          shutdown_requested_.load(), goal_handle->is_canceling(),
          execution_orchestrator_.stop_requested(sequence), stage);
  ...
  handle_cancel(const std::shared_ptr<GoalHandleExecuteMotion> goal_handle) {
    if (!action_support_) {
  ...
  void
  handle_accepted(const std::shared_ptr<GoalHandleExecuteMotion> goal_handle) {
    if (!action_support_) {
      return;
    }
    action_support_->handle_accepted(
        goal_handle,
        [this](const std::shared_ptr<GoalHandleExecuteMotion>
                   &accepted_goal_handle) { execute(accepted_goal_handle); },
        shutdown_requested_);
  }
  ...
  void execute(const std::shared_ptr<GoalHandleExecuteMotion> &goal_handle) {
    ...
    const std::string goal_id = ExecuteMotionActionSupport::goal_uuid_to_string(
    ...
    if (!ExecuteMotionActionSupport::is_supported_primitive(primitive)) {
      abort_with_message(goal_handle, started_at,
                         "unsupported primitive_type for Phase 4: " +
                             goal->primitive_type);
      return;
    }
    ...
    if (primitive == "STOP") {
      if (non_motion_executor_ &&
          non_motion_executor_->handle_stop_primitive(
              primitive, goal_id, goal_handle, started_at)) {
        return;
      }
    ...
    execution_orchestrator_.update_phase(
        goal_sequence, ExecutionPhase::kAccepted, "goal accepted");
    struct FinishGuard {
      ExecutionOrchestrator &orchestrator;
      std::uint64_t sequence;
      ~FinishGuard() { orchestrator.finish_goal(sequence, "goal finished"); }
    } finish_guard{execution_orchestrator_, goal_sequence};
    (void)finish_guard;

    RCLCPP_INFO(get_logger(),
                "execute_motion goal_seq=%lu goal_id=%s primitive=%s "
                "owner=motion_core_node",
                static_cast<unsigned long>(goal_sequence), goal_id.c_str(),
                primitive.c_str());

    const std::string initial_interrupt = make_interrupt_reason(
        shutdown_requested_.load(), goal_handle->is_canceling(),
        execution_orchestrator_.stop_requested(goal_sequence), "goal_start");
    ...
    if (ExecuteMotionActionSupport::is_non_motion_primitive(primitive)) {
      if (non_motion_executor_ &&
          non_motion_executor_->handle_non_motion_primitive(
              primitive, goal_sequence, goal, goal_handle, started_at,
              [this, goal_handle](const double progress,
      ...
      abort_with_message(goal_handle, started_at,
                         "non-motion primitive handler unavailable");
      return;
    }

    if (!dispatch_executor_ || !primitive_router_dispatch_) {
      abort_with_message(goal_handle, started_at,
                         "motion primitive executor dependencies unavailable");
      return;
    }
        ...
        *dispatch_executor_,
        ...
        [this](std::string &reason) -> bool {
          return motion_core::ensure_scene_ready(
              require_planning_scene_, scene_manager_, scene_objects_path_,
              scene_load_result_, reason);
        },
        [this]()
        ...
        [this](
          ...
          return build_current_robot_state(current_state, reason,
                                           source_joint_state_stamp);
        },
        [this, goal_handle, goal_sequence](
            moveit::planning_interface::MoveGroupInterface::Plan &plan,
            const std::string &stage)
            -> PrimitiveRouterDispatch::PlanningStageResult {
          const auto stage_result =
              plan_with_interruption(plan, goal_handle, goal_sequence, stage);
          ...
          default:
            converted.status =
                PrimitiveRouterDispatch::PlanningStatus::kFailure;
            break;
          }
        ...
        [this, goal_handle,
         goal_sequence](const std::string &stage) -> std::string {
          return make_interrupt_reason(
              shutdown_requested_.load(), goal_handle->is_canceling(),
              execution_orchestrator_.stop_requested(goal_sequence), stage);
        },
