...
bool is_large_limit_interval(const double lower_limit,
                             const double upper_limit) {
  return lower_limit < -M_PI || upper_limit > M_PI ||
         (upper_limit - lower_limit) > (2.0 * M_PI);
}

bool is_within_bounds(const double value, const double lower_limit,
                      const double upper_limit) {
  return value >= (lower_limit - kBoundsTolerance) &&
         value <= (upper_limit + kBoundsTolerance);
}
} // namespace
...
BranchPreservedAngleResult
choose_branch_preserved_angle(const double current, const double requested,
  ...
  if (!std::isfinite(current) || !std::isfinite(requested) ||
    ...
    result.reason = "branch-preserved angle selection requires finite current, "
                    "requested, and limit values";
    return result;
  }
  ...
  if (is_large_limit_interval(lower_limit, upper_limit)) {
    if (lower_limit > upper_limit) {
      result.reason = "large-limit angle helper requires ordered joint bounds "
  ...
  if (!success) {
    ...
    stream << "no legal equivalent target exists for current=" << current
           << ", requested=" << requested << ", bounds=[" << lower_limit << ", "
           << upper_limit << "]";
...
BranchPreservedJointVectorResult choose_branch_preserved_joint_vector(
  ...
  if (joint_models.size() != current.size() ||
  ...
  for (std::size_t index = 0; index < joint_models.size(); ++index) {
    ...
    if (joint_model == nullptr) {
      result.reason =
          "joint model pointer is null at index " + std::to_string(index);
      return result;
    }

    if (joint_model->getVariableCount() != 1U) {
      result.reason = "branch-preserved joint vectors currently support "
                      "single-DOF joints only; unsupported joint '" +
                      joint_model->getName() + "'";
      return result;
    }
    ...
    if (bounds.empty()) {
      result.reason =
          "joint '" + joint_model->getName() + "' does not expose bounds";
      return result;
    }
    ...
    if (!bound.position_bounded_) {
      const double delta =
          angles::shortest_angular_distance(current_value, requested_value);
    ...
    auto angle_result =
        choose_branch_preserved_angle(current_value, requested_value,
                                      bound.min_position_, bound.max_position_);
    if (!angle_result.success) {
      std::ostringstream stream;
      stream << "joint '" << joint_model->getName()
             << "' branch preservation failed: " << angle_result.reason;
      result.reason = stream.str();
      return result;
    }
