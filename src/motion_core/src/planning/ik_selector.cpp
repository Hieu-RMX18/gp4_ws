...
void IkSelector::set_move_group(
...
bool IkSelector::solve_ik(const geometry_msgs::msg::Pose &target_pose,
  ...
  const moveit::core::JointModelGroup *joint_model_group =
      initial_state->getJointModelGroup(planning_group_);
  ...
  if (seed_state.size() != expected_dof) {
    std::ostringstream stream;
    stream << "seed size mismatch for group '" << planning_group_
           << "': expected " << expected_dof << ", got " << seed_state.size();
    reason = stream.str();
    return false;
  }
  ...
  auto run_attempt = [&](const moveit::core::RobotState &reference_state,
    ...
    if (attempt_seed.size() != expected_dof) {
      std::ostringstream stream;
      stream << label << " skipped: seed size " << attempt_seed.size()
             << " does not match " << expected_dof;
    ...
    const bool solved =
        working_state.setFromIK(joint_model_group, target_pose, kIkTimeoutSec);
    if (!solved) {
      std::ostringstream stream;
      stream << label << " failed";
      attempt_logs.push_back(stream.str());
  ...
  if (attempt_count < kMaxIkAttempts) {
    ...
    if (build_fallback_seed(fallback_seed, fallback_reason)) {
      if (run_attempt(*initial_state, fallback_seed,
                      "attempt 2/fallback_seed_hook")) {
        return true;
      }
    } else {
      attempt_logs.push_back("attempt 2/fallback_seed_hook skipped: " +
                             fallback_reason);
    }
  }

  if (attempt_count < kMaxIkAttempts) {
    moveit::core::RobotStatePtr refreshed_state =
        move_group_->getCurrentState(5.0);
    if (!refreshed_state) {
      attempt_logs.push_back("attempt 3/refreshed_current_state skipped: "
                             "failed to refresh robot state");
    } else {
      std::vector<double> refreshed_seed;
      refreshed_state->copyJointGroupPositions(joint_model_group,
                                               refreshed_seed);
      if (run_attempt(*refreshed_state, refreshed_seed,
                      "attempt 3/refreshed_current_state")) {
        return true;
      }
    }
  }
  ...
  std::ostringstream stream;
  stream << "IK failed after " << attempt_count << " attempt(s); "
         << join_messages(attempt_logs);
  reason = stream.str();
  return false;
}

bool IkSelector::build_fallback_seed(std::vector<double> &fallback_seed,
  ...
  for (const auto &target_name : preferred_targets) {
    const bool available = std::find(named_targets.begin(), named_targets.end(),
                                     target_name) != named_targets.end();
    if (!available) {
      continue;
    }
  ...
  reason =
      "named-target fallback hook did not find usable 'ready' or 'home' target";
  return false;
}
} // namespace motion_core
