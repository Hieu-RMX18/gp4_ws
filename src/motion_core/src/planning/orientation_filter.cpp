...
bool OrientationFilter::normalize_and_validate(geometry_msgs::msg::Pose &pose,
                                               std::string &reason) const {
  reason.clear();

  const auto &q_in = pose.orientation;
  const double norm_sq = (q_in.x * q_in.x) + (q_in.y * q_in.y) +
...
bool OrientationFilter::validate_policy_extension(
    const geometry_msgs::msg::Pose &pose, std::string &reason) const {
  (void)pose;
  reason.clear();
  return true;
}
