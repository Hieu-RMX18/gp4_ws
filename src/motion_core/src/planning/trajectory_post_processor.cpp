...
bool TrajectoryPostProcessor::apply_totg(
    robot_trajectory::RobotTrajectory &traj, double vel_scale, double acc_scale,
  ...
  const double resolved_acc =
      resolve_scale(acc_scale, kDefaultAccelerationScaling);
...
bool TrajectoryPostProcessor::downsample_to_max_points(
    trajectory_msgs::msg::JointTrajectory &traj, std::size_t max_points,
    std::string &reason) const {
  reason.clear();
  ...
  if (max_points < 2) {
    reason =
        "cannot downsample to less than 2 points while preserving endpoints";
    return false;
  }
  ...
  const double step = static_cast<double>(point_count - 1) /
  ...
  for (std::size_t i = 0; i < max_points; ++i) {
    ...
    if (i == 0) {
      ...
      index =
          static_cast<std::size_t>(std::llround(static_cast<double>(i) * step));
      if (index <= last_index) {
