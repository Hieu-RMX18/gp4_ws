...
PrimitiveRouterDispatch::PrimitiveRouterDispatch(
  ...
  std::function<std::shared_ptr<moveit::planning_interface::MoveGroupInterface>()> move_group_provider,
  const PlannerRouter & planner_router,
  const OrientationFilter & orientation_filter,
...
: logger_(std::move(logger)),
  ...
  planner_router_(planner_router),
  orientation_filter_(orientation_filter),
  seed_manager_(seed_manager),
  ik_selector_(ik_selector),
  trajectory_post_processor_(trajectory_post_processor),
  read_current_tcp_pose_(std::move(read_current_tcp_pose))
{
}

PrimitiveRouterDispatch::PlannerSelection PrimitiveRouterDispatch::resolve_planner_selection(
...
double PrimitiveRouterDispatch::quaternion_norm_sq(const geometry_msgs::msg::Quaternion & q)
{
  return (q.x * q.x) + (q.y * q.y) + (q.z * q.z) + (q.w * q.w);
}

double PrimitiveRouterDispatch::max_abs_value(const std::vector<double> & values)
{
...
std::string PrimitiveRouterDispatch::format_joint_vector(const std::vector<double> & joints)
{
...
PrimitiveRouterDispatch::PlanningResult PrimitiveRouterDispatch::post_process_trajectory(
  ...
  const std::string & time_parameterization_failure_prefix) const
{
  ...
  robot_trajectory::RobotTrajectory robot_trajectory(move_group->getRobotModel(), kPlanningGroup);
  ...
  const bool ruckig_ok =
    trajectory_post_processor_.apply_ruckig_smoothing(
    robot_trajectory,
    velocity_scale,
    acceleration_scale,
    ruckig_reason);
  ...
  else
  {
    if (robot_trajectory.getWayPointCount() >= 2U)
    {
            ...
            totg_reason))
      {
        ...
        result.reason =
          time_parameterization_failure_prefix + failure_detail +
    ...
    {
      result.time_parameterization_note = "time_parameterization=none_single_waypoint";
    }
  }
...
PrimitiveRouterDispatch::PlanningResult PrimitiveRouterDispatch::plan_for_primitive(
  const PlanningRequest & request)
{
  ...
  const std::string planning_interrupt = request.interrupt_reason("planning_setup");
  ...
  if (primitive == "MOVE_JOINT")
  {
    ...
    if (joint_idx < 0 || static_cast<std::size_t>(joint_idx) >= request.current_joint_positions.size())
    {
      result.reason =
        "MOVE_JOINT: joint_index " + std::to_string(joint_idx) +
        " out of range [0, " + std::to_string(request.current_joint_positions.size() - 1U) + "]";
      return result;
    }

    std::vector<double> requested_joint_positions = request.current_joint_positions;
    ...
    const auto branch_result = choose_branch_preserved_joint_vector(
      request.active_joint_models,
      request.current_joint_positions,
      requested_joint_positions);
    if (!branch_result.success)
    {
      result.reason = "MOVE_JOINT branch preservation failed: " + branch_result.reason;
      return result;
    }

    log_joint_branch_selection(
      "MOVE_JOINT",
      request.goal_sequence,
    ...
    const auto plan_result = request.plan_with_interruption(plan, "MOVE_JOINT planning");
  ...
  if (effective_primitive == "MOVE_REL")
  {
    ...
    {
      result.reason = "MOVE_REL: failed to read current TCP pose: " + rel_reason;
      return result;
    }
    ...
    {
      result.reason = "MOVE_REL: current pose has invalid orientation; cannot proceed safely";
      return result;
    }
    ...
    RCLCPP_INFO(
      logger_,
      "MOVE_REL resolved: delta=(%.4f, %.4f, %.4f), current=(%.4f, %.4f, %.4f), target=(%.4f, %.4f, %.4f)",
      dx, dy, dz,
  ...
  if (pose_required)
  {
    ...
    if (effective_primitive == "PTP")
    {
      ...
      if (!branch_result.success)
      {
        result.reason = "failed to branch-preserve IK-derived PTP target: " + branch_result.reason;
        return result;
      }
  ...
  if (effective_primitive == "CIRC")
  {
    ...
    if (quaternion_norm_sq(final_pose.orientation) <= 1e-12)
    {
      ...
      {
        result.reason = "CIRC: cannot resolve orientation for final pose: " + current_reason;
        return result;
      }
  ...
  else if (effective_primitive == "HOME")
  {
    ...
    moveit::planning_interface::MoveGroupInterface::Plan plan;
    const auto plan_result = request.plan_with_interruption(plan, "HOME planning");
    if (plan_result.status == PlanningStatus::kCanceled)
    {
      result.status = PlanningStatus::kCanceled;
      result.reason = plan_result.reason;
    ...
    if (plan_result.status != PlanningStatus::kSuccess)
    {
      result.reason = "planning failed for HOME primitive: " + plan_result.reason;
      return result;
    }
    planned_trajectory_msg = plan.trajectory_;
    plan_start_state_msg = plan.start_state_;
  ...
  else if (effective_primitive == "PTP")
  {
    if (has_joint_target)
    {
      ...
      if (!branch_result.success)
      {
        result.reason = "invalid branch-preserved joint_target for PTP goal: " + branch_result.reason;
        return result;
      }
    ...
    RCLCPP_INFO(
      logger_,
      "execute_motion goal_seq=%lu PTP planning context: planner_id=%s pipeline=%s start_state=[%.3f, %.3f, %.3f, %.3f, %.3f, %.3f]",
      static_cast<unsigned long>(request.goal_sequence),
      move_group->getPlannerId().c_str(),
      move_group->getPlanningPipelineId().c_str(),
      request.current_joint_positions[0], request.current_joint_positions[1], request.current_joint_positions[2],
      request.current_joint_positions[3], request.current_joint_positions[4], request.current_joint_positions[5]);

    moveit::planning_interface::MoveGroupInterface::Plan plan;
    const auto plan_result = request.plan_with_interruption(plan, "PTP planning");
    RCLCPP_INFO(
      logger_,
    ...
    if (plan_result.status != PlanningStatus::kSuccess)
    {
      result.reason = "planning failed for PTP primitive: " + plan_result.reason;
      return result;
    }
    planned_trajectory_msg = plan.trajectory_;
    plan_start_state_msg = plan.start_state_;
  ...
  else if (effective_primitive == "LIN" || effective_primitive == "MOVE_REL")
  {
    ...
    else
    {
      ...
      if (cartesian_fraction < 0.0)
      {
        result.reason = "computeCartesianPath returned error for LIN primitive";
        return result;
      }
    }
  }
