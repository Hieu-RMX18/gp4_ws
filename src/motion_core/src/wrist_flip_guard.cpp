  ...
  std::string & reason) const
{
  ...
  for (std::size_t i = 1; i < traj.points.size(); ++i)
  {
    ...
    {
      double raw_delta = current.positions[joint_idx] - previous.positions[joint_idx];

      // V4 F1: For joint_6_t (index 5), use shortest angular distance (unwrap)
      if (joint_idx == 5)
      {
        // Normalize to [-pi, pi]
        while (raw_delta > M_PI) { raw_delta -= 2.0 * M_PI; }
        while (raw_delta < -M_PI) { raw_delta += 2.0 * M_PI; }
      }
  ...
  std::string & reason) const
{
  ...
  // More than 2 consecutive sign-flips indicates unstable wrist branch selection.
  static constexpr std::size_t kWristJointIndices[] = {3, 4, 5};
  static constexpr std::size_t kMaxConsecutiveSignFlips = 2;
  ...
  for (std::size_t joint_idx : kWristJointIndices)
  {
    ...
    for (std::size_t i = 1; i < traj.points.size(); ++i)
    {
      ...
      {
        // Sign flip: previous delta was positive, current is negative, or vice versa
        if ((prev_delta > 0.0 && delta < 0.0) || (prev_delta < 0.0 && delta > 0.0))
        {
          ...
          {
            std::ostringstream stream;
            stream << "wrist sign-flip reject: joint '" << traj.joint_names[joint_idx]
                   << "' has " << consecutive_flips
                   << " consecutive sign reversals (threshold=" << kMaxConsecutiveSignFlips
