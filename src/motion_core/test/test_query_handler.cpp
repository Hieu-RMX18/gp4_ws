...
TEST(QueryHandlerTest, RejectsUnsupportedFrameWithoutInvokingCallbacks)
{
  ...
  EXPECT_NE(response->message.find("unsupported reference_frame"), std::string::npos);
...
TEST(QueryHandlerTest, FailsClosedWhenPoseReadFails)
{
  ...
  EXPECT_FALSE(response->success);
  EXPECT_NE(response->message.find("failed to read current TCP pose"), std::string::npos);
}
...
TEST(QueryHandlerTest, ReturnsCurrentPoseOnSuccess)
{
  QueryHandler handler(
    rclcpp::get_logger("query_handler_test"),
    [&](std::string &) { return true; },
    [&](geometry_msgs::msg::PoseStamped & current_stamped, std::string &, double) {
