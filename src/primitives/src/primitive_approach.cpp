...
PrimitiveResult PrimitiveApproach::execute(const ApproachGoal &goal,
                                           MoveGroupInterface &mgi) {
  auto backend = make_move_group_linear_backend(mgi);
  return execute(goal, *backend);
}

PrimitiveResult PrimitiveApproach::execute(const ExecuteMotionGoal &goal,
                                           MoveGroupInterface &mgi) {
  if (goal.joint_target.empty()) {
    return make_failure(PrimitiveFailReason::UNKNOWN,
                        "ExecuteMotion goal for APPROACH does not provide "
                        "approach_distance; use typed ApproachGoal API");
  }
...
PrimitiveResult PrimitiveApproach::execute(const ApproachGoal &goal,
  ...
  const geometry_msgs::msg::Pose pre_approach_pose =
      offset_along_eef_local_z_world(target_pose, -goal.approach_distance);
