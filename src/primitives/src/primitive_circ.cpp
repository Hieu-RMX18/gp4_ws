...
double position_distance(const geometry_msgs::msg::Pose &a,
...
bool contains_degenerate_arc_hint(const std::string &message) {
  ...
  std::transform(
      message.begin(), message.end(), std::back_inserter(lower),
...
public:
  ...
  set_interim_path_constraint(const geometry_msgs::msg::Pose &auxiliary_point,
    ...
    // PILZ CIRC with "center" always selects the shorter arc between start and
    // goal, which is not controllable and can produce unexpected obstacle-near
    // motion. Using "interim" forces passage through an explicit on-arc point
    // and gives deterministic control over which arc segment is executed.
  ...
  void clear_path_constraints() override { mgi_.clearPathConstraints(); }
...
PrimitiveResult PrimitiveCirc::execute(const CIRCGoal &goal,
                                       MoveGroupInterface &mgi) {
  MoveGroupCircExecutionBackend backend(mgi);
  return execute(goal, backend);
}

PrimitiveResult PrimitiveCirc::execute(const ExecuteMotionGoal &goal,
                                       MoveGroupInterface &mgi) {
...
PrimitiveResult PrimitiveCirc::execute(const CIRCGoal &goal,
  ...
  PrimitiveResult result =
      backend.plan_with_pipeline(velocity_scale, acceleration_scale);
  if (!result.success &&
      (result.reason == PrimitiveFailReason::PLANNING_TIMEOUT ||
