...
public:
  explicit MoveGroupLinearExecutionBackend(MoveGroupInterface &mgi)
    ...
    // FRAME CONVENTION: all Cartesian waypoints are expressed in MoveIt's
    // planning frame (world frame).
  ...
  compute_cartesian_path(const std::vector<geometry_msgs::msg::Pose> &waypoints,
    ...
    fraction = mgi_.computeCartesianPath(waypoints, kCartesianEefStep,
                                         kCartesianJumpThreshold,
                                         planned_trajectory_, true);
  ...
  PrimitiveResult postprocess_and_validate(double velocity_scale,
    ...
    if (!quality_gate_.validate_plan(output_traj, cartesian_fraction, "LIN",
                                     quality_reason)) {
      return make_result_failure(map_quality_failure(quality_reason),
                                 "LIN quality gate rejected: " +
                                     quality_reason);
    }
...
std::unique_ptr<LinearExecutionBackend>
make_move_group_linear_backend(MoveGroupInterface &mgi) {
  return std::make_unique<MoveGroupLinearExecutionBackend>(mgi);
}

PrimitiveResult PrimitiveLin::execute(const LINGoal &goal,
                                      MoveGroupInterface &mgi) {
  auto backend = make_move_group_linear_backend(mgi);
  return execute(goal, *backend);
}

PrimitiveResult PrimitiveLin::execute(const ExecuteMotionGoal &goal,
                                      MoveGroupInterface &mgi) {
  LINGoal lin_goal;
...
PrimitiveResult PrimitiveLin::execute(const LINGoal &goal,
  ...
  return execute_waypoints(waypoints, goal.velocity_scale,
...
PrimitiveResult PrimitiveLin::execute_waypoints(
  ...
  if (fraction < motion_core::QualityGate::kMinimumCartesianFraction) {
    ...
    return make_failure(PrimitiveFailReason::CARTESIAN_FRACTION_LOW,
                        stream.str());
  }

  const LinearScalingConfig scales = backend.scaling_config();
  const double resolved_velocity = resolve_scale(
      velocity_scale, scales.default_velocity, scales.velocity_cap);
  const double resolved_acceleration = resolve_scale(
      acceleration_scale, scales.default_acceleration, scales.acceleration_cap);

  return backend.postprocess_and_validate(resolved_velocity,
                                          resolved_acceleration, fraction);
}
} // namespace primitives
