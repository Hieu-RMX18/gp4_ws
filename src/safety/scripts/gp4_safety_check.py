...
def _time_from_start_seconds(raw_value: Any) -> float:
    if isinstance(raw_value, dict):
        ...
        nanosec = raw_value.get(
            "nanosec", raw_value.get("nsec", raw_value.get("nanoseconds", 0.0))
        )
        return _as_float(sec, field="time_from_start.sec") + (
            _as_float(nanosec, field="time_from_start.nanosec") / 1_000_000_000.0
        )
...
def validate_trajectory(
    ...
    *,
    ...
    for point_index, point in enumerate(points_raw):
        ...
        time_values.append(
            _time_from_start_seconds(
                point.get("time_from_start", point.get("timeFromStart", 0.0))
            )
        )

        positions: list[float] = []
        for joint_index, (joint_name, raw_position) in enumerate(
            zip(joint_names, positions_raw)
        ):
            position = _as_float(
                raw_position, field=f"point[{point_index}].positions[{joint_index}]"
            )
            positions.append(position)
            bounds = joint_limits.get(joint_name)
            if bounds is None:
                raise ValueError(
                    f"joint {joint_name} missing from joint limits file {resolved_joint_limits_path}"
                )
        ...
        if velocities_raw is not None:
            if (
                not isinstance(velocities_raw, list)
                or len(velocities_raw) != joint_count
            ):
                raise ValueError(
            ...
            for joint_index, raw_velocity in enumerate(velocities_raw):
                try:
                    _as_float(
                        raw_velocity,
                        field=f"point[{point_index}].velocities[{joint_index}]",
                    )
        ...
        if accelerations_raw is not None:
            if (
                not isinstance(accelerations_raw, list)
                or len(accelerations_raw) != joint_count
            ):
                raise ValueError(
            ...
            for joint_index, raw_acceleration in enumerate(accelerations_raw):
                try:
                    _as_float(
                        raw_acceleration,
                        field=f"point[{point_index}].accelerations[{joint_index}]",
                    )
                except ValueError as exc:
    ...
    checks["accelerations_finite"] = _check_result(
        ...
        detail=non_finite_acceleration_detail
        or "all provided accelerations are finite",
    )
    ...
    continuity_detail = (
        "all consecutive waypoint deltas stay within WristFlipGuard thresholds"
    )
    l2_delta_detail = (
        f"all consecutive waypoint L2 deltas stay <= {MAX_WAYPOINT_L2_DELTA:.3f}"
    )
    for point_index in range(1, len(positions_per_point)):
        deltas: list[float] = []
        for joint_index, joint_name in enumerate(joint_names):
            raw_delta = (
                positions_per_point[point_index][joint_index]
                - positions_per_point[point_index - 1][joint_index]
            )
            if joint_index == 5:
                raw_delta = _shortest_angular_delta(raw_delta)
            delta = abs(raw_delta)
            deltas.append(raw_delta)
            threshold = JOINT_DELTA_THRESHOLDS.get(
                joint_index, JOINT_DELTA_THRESHOLDS[0]
            )
        ...
        if l2_delta > MAX_WAYPOINT_L2_DELTA and l2_delta_detail.startswith(
            "all consecutive"
        ):
            l2_delta_detail = (
    ...
    checks["continuity_thresholds"] = _check_result(
        ...
        thresholdsRad={
            str(index): threshold for index, threshold in JOINT_DELTA_THRESHOLDS.items()
        },
    )
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "trajectory_path", help="Path to a JSON or YAML joint trajectory file."
    )
    parser.add_argument(
        "--joint-limits",
