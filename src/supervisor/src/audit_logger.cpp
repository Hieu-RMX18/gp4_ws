...
std::string default_audit_log_path()
{
  return "/tmp/gp4_audit";
}
...
Json::Value goal_status_array_to_json(const action_msgs::msg::GoalStatusArray & msg)
{
  Json::Value output(Json::objectValue);
...
Json::Value execute_motion_feedback_to_json(const ExecuteMotionFeedbackMessage & msg)
{
  Json::Value output(Json::objectValue);
...
Json::Value validate_command_request_to_json(const ValidateCommandRequest & msg)
{
  Json::Value output(Json::objectValue);
...
Json::Value validate_command_response_to_json(const ValidateCommandResponse & msg)
{
  Json::Value output(Json::objectValue);
...
Json::Value robot_readiness_to_json(const interfaces::msg::RobotReadiness & msg)
{
  Json::Value output(Json::objectValue);
...
MessageT deserialize_message(const rclcpp::SerializedMessage & serialized_message)
{
  MessageT message;
  ...
  logger_(node.get_logger())
{
  serialization_format_ = rmw_get_serialization_format() == nullptr ?
    "cdr" : std::string(rmw_get_serialization_format());
  audit_path_ = declare_or_get_parameter<std::string>(
    node_,
    "audit_log_path",
    default_audit_log_path());
  max_queue_depth_ = static_cast<std::size_t>(declare_or_get_parameter<int64_t>(
      node_,
      "audit_logger_max_queue_depth",
      2048));
  ...
  RCLCPP_INFO(
    logger_,
    "audit_logger recording to bag='%s' and jsonl='%s'",
    bag_uri_.string().c_str(),
    jsonl_path_.string().c_str());
}
...
void AuditLogger::initialize_output_paths()
{
  ...
  if (!jsonl_stream_.is_open())
  {
    throw std::runtime_error("failed to open audit JSONL file: " + jsonl_path_.string());
  }
}
...
void AuditLogger::initialize_subscriptions()
{
  ...
  for (const auto & spec : topic_specs_)
  {
    subscriptions_.push_back(node_.create_generic_subscription(
        ...
        spec.type_name,
        spec.qos,
        [this, spec](std::shared_ptr<rclcpp::SerializedMessage> serialized_message) {
  ...
  std::shared_ptr<rclcpp::SerializedMessage> serialized_message)
{
...

  {
    ...
    if (event_queue_.size() >= max_queue_depth_)
    {
      ...
      RCLCPP_WARN_THROTTLE(
        ...
        *node_.get_clock(),
        5000,
        "audit_logger queue full; dropping oldest event to remain non-blocking");
    }
  ...
  while (callback_duration > current_max &&
    !max_callback_latency_ns_.compare_exchange_weak(
      current_max,
      callback_duration))
  {
  }
  ...
  switch (event.stream_id)
  {
    case StreamId::kExecuteStatus:
      record["payload"] = goal_status_array_to_json(
        deserialize_message<action_msgs::msg::GoalStatusArray>(*event.serialized_message));
      break;

    case StreamId::kExecuteFeedback:
      record["payload"] = execute_motion_feedback_to_json(
        deserialize_message<ExecuteMotionFeedbackMessage>(*event.serialized_message));
      break;
    ...
    case StreamId::kValidateResponse:
      record["payload"] = validate_command_response_to_json(
        deserialize_message<ValidateCommandResponse>(*event.serialized_message));
      break;

    case StreamId::kHwReady:
      record["payload"] = robot_readiness_to_json(
        deserialize_message<interfaces::msg::RobotReadiness>(*event.serialized_message));
      break;
  }
