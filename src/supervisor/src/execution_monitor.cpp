...
diagnostic_msgs::msg::KeyValue make_key_value(const std::string &key,
...
std::string
ExecutionMonitor::state_to_string(const ExecutionMonitorState state) {
  switch (state) {
...
ExecutionMonitor::ExecutionMonitor(rclcpp::Node &node)
  ...
  const auto send_goal_response_topic = declare_or_get_parameter<std::string>(
      node, "execute_motion_send_goal_response_topic",
  ...
  alert_pub_ = node.create_publisher<diagnostic_msgs::msg::DiagnosticStatus>(
      alert_topic, rclcpp::QoS(10).reliable());

  send_goal_request_sub_ =
      node.create_subscription<ExecuteMotionSendGoalRequest>(
          send_goal_request_topic, rclcpp::ServicesQoS(),
          std::bind(&ExecutionMonitor::send_goal_request_callback, this,
                    std::placeholders::_1));

  send_goal_response_sub_ =
      node.create_subscription<ExecuteMotionSendGoalResponse>(
          send_goal_response_topic, rclcpp::ServicesQoS(),
          std::bind(&ExecutionMonitor::send_goal_response_callback, this,
                    std::placeholders::_1));

  feedback_sub_ = node.create_subscription<ExecuteMotionFeedbackMessage>(
      execute_motion_action_name + "/_action/feedback",
      rclcpp::QoS(10).reliable(),
      std::bind(&ExecutionMonitor::feedback_callback, this,
                std::placeholders::_1));

  status_sub_ = node.create_subscription<action_msgs::msg::GoalStatusArray>(
      execute_motion_action_name + "/_action/status",
      rclcpp::QoS(10).reliable(),
      std::bind(&ExecutionMonitor::status_callback, this,
                std::placeholders::_1));
  ...
  heartbeat_timer_ = node.create_wall_timer(
      std::chrono::milliseconds(
          alert_heartbeat_period_ms > 0 ? alert_heartbeat_period_ms : 1000),
  ...
  RCLCPP_INFO(logger_,
              "execution_monitor attached to %s; nominal_duration_sec=%.3f "
              "timeout_multiplier=%.2f",
              execute_motion_action_name.c_str(), nominal_duration_sec_,
              timeout_multiplier_);
}
...
void ExecutionMonitor::send_goal_request_callback(
    const ExecuteMotionSendGoalRequest::SharedPtr msg) {
  if (!msg) {
    return;
  }

  const auto received_time = clock_->now();
  const auto tracked_goal =
      build_tracked_goal(msg->goal_id, msg->goal, received_time);
...
void ExecutionMonitor::send_goal_response_callback(
...
void ExecutionMonitor::feedback_callback(
    const ExecuteMotionFeedbackMessage::SharedPtr msg) {
  if (!msg) {
    return;
  }
...
void ExecutionMonitor::status_callback(
...

  {
    ...
    snapshot_.active_goal_count = static_cast<std::size_t>(
        std::count_if(msg->status_list.begin(), msg->status_list.end(),
    ...
    if (snapshot_.active_goal_count > 1U) {
      RCLCPP_WARN(logger_,
                  "execution_monitor observed %zu active action goals; GP4 "
                  "execution is expected to remain single-goal.",
...
void ExecutionMonitor::timeout_check_callback() {
...

  {
    ...
    const auto elapsed_sec =
        (clock_->now() - active_goal_->accepted_time).seconds();
  ...
  if (publish_timeout) {
    publish_alert(diagnostic_msgs::msg::DiagnosticStatus::WARN,
                  "execute_motion exceeded allowed duration",
                  "elapsed wall time exceeded 2x expected duration");
  }
}

void ExecutionMonitor::promote_pending_goal_to_active(
    const rclcpp::Time &accepted_time) {
  if (!pending_goal_.has_value()) {
    return;
  }
...
void ExecutionMonitor::finalize_goal(const bool success,
...

  {
    ...
    const auto start_time = tracked_goal->accepted_time.nanoseconds() > 0
                                ? tracked_goal->accepted_time
                                : tracked_goal->requested_time;
    const auto elapsed_sec =
        std::max(0.0, (completed_time - start_time).seconds());

    state_ = success ? ExecutionMonitorState::kSuccess
                     : ExecutionMonitorState::kFailed;
    snapshot_.current_state = state_to_string(state_);
    snapshot_.active_goal_id = tracked_goal->goal_id;
    snapshot_.active_velocity_scale = tracked_goal->velocity_scale;
    snapshot_.expected_duration_sec = tracked_goal->expected_duration_sec;
  ...
  publish_alert(success ? diagnostic_msgs::msg::DiagnosticStatus::OK
                        : diagnostic_msgs::msg::DiagnosticStatus::ERROR,
                success ? "execute_motion completed successfully"
...
void ExecutionMonitor::publish_heartbeat() {
  ...
  publish_alert(heartbeat_level, heartbeat_message, "heartbeat");
}

void ExecutionMonitor::publish_alert(const uint8_t level,
  ...
  alert.values.push_back(
      make_key_value("active_goal_id", local_snapshot.active_goal_id));
  alert.values.push_back(
      make_key_value("consecutive_failure_count",
                     std::to_string(local_snapshot.consecutive_failure_count)));
  alert.values.push_back(make_key_value(
      "velocity_scale", std::to_string(local_snapshot.active_velocity_scale)));
  alert.values.push_back(
      make_key_value("expected_duration_sec",
                     std::to_string(local_snapshot.expected_duration_sec)));
  alert.values.push_back(
      make_key_value("allowed_duration_sec",
                     std::to_string(local_snapshot.allowed_duration_sec)));
  alert.values.push_back(
      make_key_value("last_execution_time_sec",
...
ExecutionMonitor::TrackedGoal ExecutionMonitor::build_tracked_goal(
    ...
    const ExecuteMotionGoal &goal, const rclcpp::Time &received_time) const {
  ...
  tracked_goal.expected_duration_sec =
      nominal_duration_sec_ / tracked_goal.velocity_scale;
  tracked_goal.allowed_duration_sec =
      tracked_goal.expected_duration_sec * timeout_multiplier_;
...
std::string ExecutionMonitor::goal_id_to_string(
    const unique_identifier_msgs::msg::UUID &goal_id) {
...
double
ExecutionMonitor::sanitize_velocity_scale(const double velocity_scale) const {
