...
using ExecuteMotionSendGoalRequest = interfaces::action::ExecuteMotion_SendGoal_Request;
using ExecuteMotionSendGoalResponse = interfaces::action::ExecuteMotion_SendGoal_Response;
...
std::string goal_id_to_string(const unique_identifier_msgs::msg::UUID & goal_id)
{
...
protected:
  void SetUp() override
  {
    ...
    alert_sub_ = node_->create_subscription<diagnostic_msgs::msg::DiagnosticStatus>(
      "/supervisor/alerts",
    ...
    send_goal_request_pub_ = node_->create_publisher<ExecuteMotionSendGoalRequest>(
      "/execute_motion/_action/send_goal/_request",
      rclcpp::ServicesQoS());
    send_goal_response_pub_ = node_->create_publisher<ExecuteMotionSendGoalResponse>(
  ...
  void publish_goal_request(const unique_identifier_msgs::msg::UUID & goal_id, const double velocity_scale)
  {
  ...
  void publish_status(const unique_identifier_msgs::msg::UUID & goal_id, const int8_t status_code)
  {
  ...
  rclcpp::Subscription<diagnostic_msgs::msg::DiagnosticStatus>::SharedPtr alert_sub_;
  rclcpp::Publisher<ExecuteMotionSendGoalRequest>::SharedPtr send_goal_request_pub_;
  rclcpp::Publisher<ExecuteMotionSendGoalResponse>::SharedPtr send_goal_response_pub_;
...
TEST_F(ExecutionMonitorFixture, NormalCompletionPathPublishesOkAlert)
{
  ...
  ASSERT_TRUE(wait_for_alert_message(
      "execute_motion completed successfully",
...
TEST_F(ExecutionMonitorFixture, ConsecutiveFailuresPublishWarnAlert)
{
  for (uint8_t index = 0U; index < 3U; ++index)
  {
    ...
    ASSERT_TRUE(wait_for_condition(
        executor_,
        [this, index]() {
          return monitor_->snapshot().consecutive_failure_count == static_cast<uint32_t>(index + 1U);
        },
        std::chrono::seconds(5)));
  }
